...
const [timezoneLoading, setTimezoneLoading] = useState(false);
  ...
  const [birthLocationLoading, setBirthLocationLoading] = useState(false);
  const birthLocationRef = useRef(null);
  const [transitLocationSuggestions, setTransitLocationSuggestions] = useState([]);
const [showTransitSuggestions, setShowTransitSuggestions] = useState(false);
const [transitLocationLoading, setTransitLocationLoading] = useState(false);
const transitLocationRef = useRef(null);
...
const DEFAULT_CHART_TYPES = ["Natal", "Progressed", "Transit"];
const DEFAULT_DISPLAY_POINTS = ["AS", "MC", "DS", "IC"];


const natalPlanets = chartData ? 
  Object.entries(chartData.natal.planets)
    .filter(([name]) => {
      ...
      if (shouldShowAllPlanets) {
        return DEFAULT_PLANETS.includes(name);
      }
      
      return displayPlanets.includes(name);
    })
    .map(([name, data]) => ({
      name,
    ...
    const progressedPlanets = chartData ? 
  Object.entries(chartData.progressed.planets)
    .filter(([name]) => {
      const displayPlanets = settings?.wheelSettings?.displayPlanets;
    ...
    const transitPlanets = chartData ? 
    Object.entries(chartData.transit.planets)
      .filter(([name]) => {
        const displayPlanets = settings?.wheelSettings?.displayPlanets;
      ...
      const shouldShowChart = (chartType) => {
        const chartTypes = settings?.graphSettings?.chartTypes?.length > 0 
          ? settings.graphSettings.chartTypes 
          : DEFAULT_CHART_TYPES;
  ...
  chartData.aspects.natal_to_transit
    .filter(aspect => {
    ...
    .slice(0, 5)
    .map(aspect => ({
      from: parseFloat(chartData.natal.planets[aspect.planet1].longitude),
      to: parseFloat(chartData.transit.planets[aspect.planet2].longitude),
      color: getAspectColor(aspect.aspect)
    })) : [];
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (transitLocationRef.current && !transitLocationRef.current.contains(event.target)) {
  ...
  useEffect(() => {
    const fetchTransitLocationSuggestions = async () => {
      if (!formData.location || formData.location.length < 3) {
        setTransitLocationSuggestions([]);
        return;
      }
  
      setTransitLocationLoading(true);
      try {
        ...
        setTransitLocationSuggestions(formattedSuggestions);
        setShowTransitSuggestions(true);
      } catch (error) {
        console.error('Error fetching transit location suggestions:', error);
        setTransitLocationSuggestions([]);
      } finally {
        setTransitLocationLoading(false);
      }
    };
  
    const debounceTimer = setTimeout(fetchTransitLocationSuggestions, 300);
    return () => clearTimeout(debounceTimer);
  }, [formData.location]);
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (birthLocationRef.current && !birthLocationRef.current.contains(event.target)) {
  ...
  useEffect(() => {
    const fetchBirthLocationSuggestions = async () => {
      if (!formData.birthLocation || formData.birthLocation.length < 3) {
        setBirthLocationSuggestions([]);
        return;
      }

      setBirthLocationLoading(true);
      try {
        const response = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(formData.birthLocation)}&limit=5&addressdetails=1`,
          {
            headers: {
              'User-Agent': 'AstrologyApp/1.0'
            }
        ...
        setBirthLocationSuggestions(formattedSuggestions);
        setShowBirthSuggestions(true);
      } catch (error) {
        console.error('Error fetching location suggestions:', error);
        setBirthLocationSuggestions([]);
      } finally {
        setBirthLocationLoading(false);
      }
    };

    const debounceTimer = setTimeout(fetchBirthLocationSuggestions, 300);
    return () => clearTimeout(debounceTimer);
  }, [formData.birthLocation]);

  const handleBirthLocationInputChange = (value) => {
    setFormData(prev => ({ ...prev, birthLocation: value }));
    setShowBirthSuggestions(true);
  };

  const handleTransitLocationInputChange = (value) => {
    setFormData(prev => ({ ...prev, location: value }));
    setShowTransitSuggestions(true);
  };
  
  const handleSelectTransitLocationSuggestion = (suggestion) => {
    setFormData(prev => ({
      ...prev,
  ...
  const handleRemoveChart = async(chartId) => {
    ...
    if (window.confirm('Are you sure you want to remove this chart?')) {
      ...
      try{
        let response=await axios.delete(`${BASE_URL}/transit-chart/${chartId}`)
...
if(e?.response?.data?.error){
  ...
  const handleSelectBirthLocationSuggestion = (suggestion) => {
    setFormData(prev => ({
      ...prev,
      birthLocation: suggestion.display,
      birthLatitude: suggestion.lat,
      birthLongitude: suggestion.lon
    }));
    setShowBirthSuggestions(false);
    setBirthLocationSuggestions([]);
  };
  ...
  const generatePDF = () => {
    ...
    printWindow.document.write(`
      <html>
        <head>
          <meta charset="UTF-8">
          <title>Transit Chart - ${formData.birthDate} - ${new Date().toLocaleDateString()}</title>
          <style>
            body { 
              margin: 0;
              padding: 20px;
        ...
        <body>
          <h1>Progressed & Transit Chart</h1>
          <div class="info">
            <strong>Birth:</strong> ${formData.birthDate} ${formData.birthTime} • ${formData.birthLocation || 'Location not specified'}<br>
            <strong>Transit:</strong> ${formData.day} ${formData.month} ${formData.year} • ${formData.hour}:${formData.minute} • ${formData.location || 'Location not specified'}
          </div>
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      if (result.success) {
        // ADD THIS: Store birth and transit info before clearing formData
  ...
  const handleSave = async() => {
    try{
      ...
      let saveChart = {
        ...
        transitInfo: chartResponse.transitInfo || {},
      ...
      console.log("Saving chart data:", saveChart); 
      ...
      toast.success("Chart saved sucessfully",{containerId:"transitchart"})
      setError(null)
      setFormData({
        name: '',
        day: '',
...
//     const natalHouses = chartData.natal?.houses || [];
    
    
//     let saveChart = {
//       chart_type: chartData.chart_type || "Transit (Triwheel)",
//       chartName: formData.chartName,
//       chartname: 'transit',
...
//       },
//       natal: {
//         planets: chartData.natal?.planets || {},
//         ascendant: chartData.natal?.ascendant || {},
//         houses: natalHouses
//       },
//       progressed: {
//         planets: chartData.progressed?.planets || {},
//         ascendant: chartData.progressed?.ascendant || {},
       
//         houses: (chartData.progressed?.houses && chartData.progressed.houses.length > 0) 
//           ? chartData.progressed.houses 
//           : natalHouses,
//         progression_info: chartData.progressed?.progression_info || {}
//       },
//       transit: {
//         planets: chartData.transit?.planets || {},
//         ascendant: chartData.transit?.ascendant || {},
//         date: chartData.transit?.date || '',
//         time: chartData.transit?.time || '',
       
//         houses: (chartData.transit?.houses && chartData.transit.houses.length > 0) 
//           ? chartData.transit.houses 
//           : natalHouses
//       }
//     };
...
const getHouseInterpretation = (houseNum) => {
  ...
  const getTransitChart=async(pageNum = 1, append = false)=>{
    try{
      ...
      let token=localStorage.getItem('token')
      token=JSON.parse(token)
      let response=await axios.get(`${BASE_URL}/getTransitChart?page=${pageNum}&limit=5`,{headers:{
        Authorization:`Bearer ${token}`
      }})
      console.log(response.data)
      console.log("man")
  ...
  <ToastContainer containerId={"transitchart"}/>
...
{shouldShowChart('Progressed') && progressedPlanets.map((planet, index) => {
  const angle = planet.degree - 90;
  const radius = 210;
  const x = 400 + Math.cos(angle * Math.PI / 180) * radius;
  const y = 400 + Math.sin(angle * Math.PI / 180) * radius;
...
{shouldShowChart('Transit') && transitPlanets.map((planet, index) => {
  const angle = planet.degree - 90;
  const radius = 335;
  const x = 400 + Math.cos(angle * Math.PI / 180) * radius;
  const y = 400 + Math.sin(angle * Math.PI / 180) * radius;
...
{chartData && chartData.progressed?.progression_info && (
  <div className="mb-4 p-3 bg-blue-50 border border-blue-200 text-sm">
    <div><strong>Birth Time Sent:</strong> {formData.birthDate} {formData.birthTime}</div>
    <div><strong>Progressed Time Received:</strong> {chartData.progressed.progression_info.birth_date} {chartData.progressed.progression_info.birth_time}</div>
    <div><strong>Age:</strong> {chartData.progressed.progression_info.age_in_years} years</div>
  </div>
)}
...
{chartData && (() => {
  const displayPoints = settings?.graphSettings?.displayPoints?.length > 0 
    ? settings.graphSettings.displayPoints 
    ...
    <>
      {displayPoints.includes('AS') && (
        <g>
          ...
          <text x="555" y="390" fontSize="16" fontWeight="bold">
      ...
      {displayPoints.includes('DS') && (
        <g>
          <line x1="400" y1="400" x2="230" y2="400" stroke="#000" strokeWidth="3" />
          <text x="245" y="390" fontSize="16" fontWeight="bold">DS</text>
        </g>
      )}

      {displayPoints.includes('IC') && (
        <g>
          <line x1="400" y1="400" x2="400" y2="570" stroke="#000" strokeWidth="3" />
          <text x="385" y="555" fontSize="16" fontWeight="bold">IC</text>
        </g>
      )}
    </>
  );
      ...
      <div className="bg-white rounded-lg shadow-sm p-6 max-w-xl mx-auto">
        ...
        <div className="space-y-4">
          <div className="border-b pb-4 mb-4">
            ...
            <div className="flex items-center gap-4 flex-col md:flex-row mb-3">
              <label className="w-48 md:text-right font-medium">Birth Date:</label>
              <input
                type="date"
            ...
            <div className="flex items-center gap-4 flex-col md:flex-row mb-3">
              <label className="w-48 md:text-right font-medium">Birth Time:</label>
              <input
                type="time"
            ...
            <div className="flex items-center gap-4 flex-col md:flex-row mb-3">
              <label className="w-48 md:text-right font-medium">Birth Location:</label>
              <div className="flex-1 relative" ref={birthLocationRef}>
                <input
                  type="text"
                  value={formData.birthLocation}
                  onChange={(e) => handleBirthLocationInputChange(e.target.value)}
                  onFocus={() => birthLocationSuggestions.length > 0 && setShowBirthSuggestions(true)}
                  className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300"
                  placeholder="Enter birth location..."
                  autoComplete="off"
                />
                
                {birthLocationLoading && (
                ...
                {showBirthSuggestions && birthLocationSuggestions.length > 0 && (
                  <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded shadow-lg max-h-60 overflow-y-auto mt-1">
                    {birthLocationSuggestions.map((suggestion, index) => (
                      <li
                        key={index}
                        onClick={() => handleSelectBirthLocationSuggestion(suggestion)}
                ...
                {showBirthSuggestions && !birthLocationLoading && formData.birthLocation.length >= 3 && birthLocationSuggestions.length === 0 && (
    ...
    >
      {timezoneLoading ? (
        ...
        timezoneList.map((tz) => (
          ...
          <h3 className="font-semibold mb-3">Transit Information</h3>
...
/>
  ...
  <div className="flex-1 relative" ref={transitLocationRef}>
    <input
      type="text"
      value={formData.location}
      onChange={(e) => handleTransitLocationInputChange(e.target.value)}
      onFocus={() => transitLocationSuggestions.length > 0 && setShowTransitSuggestions(true)}
      className="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-300"
      placeholder="Enter transit location..."
      autoComplete="off"
    />
    
    {transitLocationLoading && (
      <div className="absolute right-3 top-3 text-gray-400">
        <svg className="animate-spin h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
    ...
    {showTransitSuggestions && transitLocationSuggestions.length > 0 && (
      <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded shadow-lg max-h-60 overflow-y-auto mt-1">
        {transitLocationSuggestions.map((suggestion, index) => (
          <li
            key={index}
            onClick={() => handleSelectTransitLocationSuggestion(suggestion)}
            className="px-3 py-2 hover:bg-purple-50 cursor-pointer border-b border-gray-100 last:border-b-0"
          >
            <div className="text-sm text-gray-900">{suggestion.display}</div>
          </li>
    ...
    {showTransitSuggestions && !transitLocationLoading && formData.location.length >= 3 && transitLocationSuggestions.length === 0 && (
      <div className="absolute z-10 w-full bg-white border border-gray-300 rounded shadow-lg mt-1 px-3 py-2 text-sm text-gray-500">
        No locations found
      </div>
    )}
...
{savedCharts.map((val, i) => (
    ...
    <div 
      ...
      onClick={() => {
        ...
        const formattedChartData = {
          natal: val.natal || {},
          progressed: {
            planets: val.progressed?.planets || {},
            ascendant: val.progressed?.ascendant || {},
            houses: val.progressed?.houses || [],
            // Add progression_info here for backward compatibility
            progression_info: val.progression_info || val.progressed?.progression_info || {
              birth_date: 'N/A',
              birth_time: 'N/A',
              age_in_years: 'N/A'
            }
          ...
          transit: {
            planets: val.transit?.planets || {},
            ascendant: val.transit?.ascendant || {},
            houses: val.transit?.houses || []
          },
          aspects: {
            natal_to_transit: val.aspects?.natal_to_transit || [],
            natal_to_progressed: val.aspects?.natal_to_progressed || [],
            progressed_to_transit: val.aspects?.progressed_to_transit || []
          },
          birthInfo: val.birthInfo || {},
          transitInfo: val.transitInfo || {}
        };
        ...
        >
          ...
          >
            ...
            <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white p-6 flex justify-between items-center">
              <div>
                <h2 className="text-2xl font-bold">Transit & Progression Report</h2>
  ...
  <p className="text-sm text-gray-700">Date: {chartData.birthInfo?.birthDate || 'N/A'}</p>
  <p className="text-sm text-gray-700">Time: {chartData.birthInfo?.birthTime || 'N/A'}</p>
  <p className="text-sm text-gray-700">Location: {chartData.birthInfo?.birthLocation || 'N/A'}</p>
</div>
               
                  <div className="bg-pink-50 p-4 rounded-lg">
  <h4 className="font-semibold text-pink-900 mb-2">Transit Information</h4>
  <p className="text-sm text-gray-700">
    Date: {chartData.transitInfo?.day || 'N/A'} {chartData.transitInfo?.month || ''} {chartData.transitInfo?.year || ''}
  </p>
  <p className="text-sm text-gray-700">
    Time: {chartData.transitInfo?.hour || 'N/A'}:{chartData.transitInfo?.minute || 'N/A'}
  </p>
  <p className="text-sm text-gray-700">Location: {chartData.transitInfo?.location || 'N/A'}</p>
</div>
                </div>
              </div>
...
{/* Other Natal Planets with detailed interpretations */}
<div className="mb-8">
  <h3 className="text-xl font-bold text-gray-800 mb-4 border-b-2 border-purple-500 pb-2">
    Other Natal Planetary Positions
  </h3>
...
<div className="mb-8">
      ...
      <div key={house.house} className="bg-gray-50 p-3 rounded-lg">
        ...
        <p className="text-sm text-gray-700 mt-2">
          {getHouseInterpretation(house.house)}
             ...
             {chartData?.natal?.planets && (() => {
  const displayPlanets = settings?.wheelSettings?.displayPlanets?.length > 0 
    ? settings.wheelSettings.displayPlanets 
    : DEFAULT_PLANETS;
  
  const filteredPlanets = Object.entries(chartData.natal.planets)
    .filter(([planet]) => displayPlanets.includes(planet));
  ...
  return filteredPlanets.length > 0 && (
    <div className="mb-8">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {filteredPlanets.map(([planet, data]) => (
          <div key={planet} className="bg-gray-50 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-2xl" style={{ color: planetColors[planet] || '#000' }}>
                {getPlanetSymbol(planet)}
              </span>
              <div>
             ...
             {chartData?.progressed?.planets && (() => {
  const displayPlanets = settings?.wheelSettings?.displayPlanets?.length > 0 
    ? settings.wheelSettings.displayPlanets 
    : DEFAULT_PLANETS;
  
  const filteredPlanets = Object.entries(chartData.progressed.planets)
    .filter(([planet]) => displayPlanets.includes(planet));
  ...
  return filteredPlanets.length > 0 && (
    <div className="mb-8">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {filteredPlanets.map(([planet, data]) => (
          <div key={planet} className="bg-blue-50 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-2xl" style={{ color: planetColors[planet] || '#000' }}>
                {getPlanetSymbol(planet)}
              </span>
              <div>
                <h4 className="font-bold text-gray-800">{planet} (Progressed)</h4>
                <p className="text-xs text-gray-600">
                  {data.position} in {data.sign}
                </p>
              </div>
            </div>
            <p className="text-xs text-gray-700 mt-2">
              Shows current life developments and how your {planet} energy is evolving.
            </p>
          </div>
        ))}
      </div>
              ...
              {chartData?.transit?.planets && (() => {
  const displayPlanets = settings?.wheelSettings?.displayPlanets?.length > 0 
    ? settings.wheelSettings.displayPlanets 
    : DEFAULT_PLANETS;
  
  const filteredPlanets = Object.entries(chartData.transit.planets)
    .filter(([planet]) => displayPlanets.includes(planet));
  
  return filteredPlanets.length > 0 && (
    <div className="mb-8">
      <h3 className="text-xl font-bold text-gray-800 mb-4 border-b-2 border-pink-500 pb-2">
        Transit Planetary Positions
      </h3>
      <div className="grid md:grid-cols-2 gap-4">
        {filteredPlanets.map(([planet, data]) => (
          <div key={planet} className="bg-pink-50 p-3 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-2xl" style={{ color: planetColors[planet] || '#000' }}>
                {getPlanetSymbol(planet)}
              </span>
              <div>
                <h4 className="font-bold text-gray-800">{planet} (Transit)</h4>
                <p className="text-xs text-gray-600">
                  {data.position} in {data.sign}
                </p>
              </div>
            ...
            <p className="text-xs text-gray-700 mt-2">
              Current planetary influence affecting your life circumstances.
            </p>
          </div>
        ))}
      </div>
              ...
              {chartData?.aspects?.natal_to_transit && (() => {
  const enabledAspects = settings?.graphAspects?.enabled?.length > 0 
    ? settings.graphAspects.enabled 
    : DEFAULT_ASPECTS;
  
  const showTransitingToNatal = settings?.graphAspects?.types?.transitingToNatal ?? true;
  
  const filteredAspects = chartData.aspects.natal_to_transit
    .filter(aspect => enabledAspects.includes(aspect.aspect));
  
  return showTransitingToNatal && filteredAspects.length > 0 && (
    <div className="mb-8">
      <h3 className="text-xl font-bold text-gray-800 mb-4 border-b-2 border-green-500 pb-2">
        Natal to Transit Aspects ({filteredAspects.length})
      </h3>
      <div className="space-y-3">
        {filteredAspects.slice(0, 15).map((aspect, idx) => (
          <div key={idx} className="bg-green-50 p-3 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-semibold text-gray-800">
                Natal {aspect.planet1} {aspect.aspect} Transit {aspect.planet2}
              </h4>
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              Your natal {aspect.planet1} is forming a {aspect.aspect.toLowerCase()} with transiting {aspect.planet2}, indicating {
                aspect.aspect === 'Conjunction' ? 'a powerful blending of energies and new beginnings' : 
                aspect.aspect === 'Opposition' ? 'tension requiring balance and awareness of external factors' : 
                aspect.aspect === 'Trine' ? 'harmonious opportunities and easy flow of energy' : 
                aspect.aspect === 'Square' ? 'dynamic challenges prompting action and growth' : 
                aspect.aspect === 'Sextile' ? 'opportunities requiring some effort to activate' :
                'significant planetary interaction'
              }.
            </p>
          </div>
        ))}
           ...
           {chartData?.aspects?.natal_to_progressed && (() => {
  const enabledAspects = settings?.graphAspects?.enabled?.length > 0 
    ? settings.graphAspects.enabled 
    : DEFAULT_ASPECTS;
  
  const showProgressedToNatal = settings?.graphAspects?.types?.progressedToNatal ?? true;
  
  const filteredAspects = chartData.aspects.natal_to_progressed
    .filter(aspect => enabledAspects.includes(aspect.aspect));
  
  return showProgressedToNatal && filteredAspects.length > 0 && (
    <div className="mb-8">
      <h3 className="text-xl font-bold text-gray-800 mb-4 border-b-2 border-indigo-500 pb-2">
        Natal to Progressed Aspects ({filteredAspects.length})
      </h3>
      <div className="space-y-3">
        {filteredAspects.slice(0, 10).map((aspect, idx) => (
          <div key={idx} className="bg-indigo-50 p-3 rounded-lg">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-semibold text-gray-800">
                Natal {aspect.planet1} {aspect.aspect} Progressed {aspect.planet2}
              </h4>
              <span className="text-sm text-gray-600">
                Orb: {aspect.orb}° {aspect.exact && '✓ Exact'}
              </span>
            ...
            <p className="text-sm text-gray-700 leading-relaxed">
              This {aspect.aspect.toLowerCase()} shows how your evolving nature is interacting with your core self.
            </p>
          </div>
        ))}
      </div>
...
function getAspectColor(aspectName) {
  ...
  return colors[aspectName] || '#888888';
}
